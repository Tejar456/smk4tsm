<template>
  ...
  <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <!-- Card -->
      <div data-aos="fade-up" v-for="(articel, i) in articels" :key="i" :data-aos-delay="i * 200">
        <a class="group flex flex-col h-full border border-gray-200 hover:border-transparent hover:shadow-lg focus:outline-none focus:border-transparent focus:shadow-lg transition duration-300 rounded-xl p-5"
          ...
          <div class="aspect-w-16 aspect-h-11">
            <img class="w-full h-60 object-cover rounded-xl" :src="articel.cover" alt="cover" />
          </div>
          <div class="my-6">
            <h3 class="text-xl font-semibold text-gray-800">{{ articel.judul }}</h3>
